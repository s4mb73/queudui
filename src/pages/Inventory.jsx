    ...
    <div className="fade-up">
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: "#9ca3af", marginTop: 3 }}>{Object.keys(eventGroups).length} events · {totalStock} tickets available</div>
        </div>
      ...
      <div style={{ background: "#ffffff", border: "0.5px solid #e8e8ec", borderRadius: 10, padding: "10px 14px", display: "flex", alignItems: "center", gap: 10, marginBottom: 10, flexWrap: "wrap", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
        ...
        <div style={{ display: "flex", gap: 4 }}>
          {["All","Sport","Concert"].map(c => (
            <button key={c} onClick={() => setFilterCat(c)} style={{ padding: "4px 10px", borderRadius: 20, border: `1px solid ${filterCat === c ? "#1a3a6e" : "#e8e8ec"}`, background: filterCat === c ? "#1a3a6e" : "transparent", color: filterCat === c ? "#ffffff" : "#6b7280", fontSize: 11, fontWeight: 600, cursor: "pointer", fontFamily: FONT }}>
              {c}
            </button>
          ))}
        </div>
      ...
      <div style={{ ...card, overflow: "hidden" }}>
        ...
        {tickets.length === 0 ? (
          ...
          <div style={{ padding: "48px 18px", textAlign: "center" }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 600, color: "#111827" }}>No results for "{searchQ}"</div>
            <div style={{ fontSize: 12, marginTop: 4, color: "#9ca3af" }}>Try a different event name, section, or order ref</div>
            <button className="ghost-btn" style={{ marginTop: 12 }} onClick={() => { setSearchQ(""); setFilterCat("All"); setFilterStatus("All"); }}>Clear filters</button>
          </div>
                ...
                return (
                  <div key={oKey} style={{ borderTop: "0.5px solid #f0f0f3" }}>
                    <div onClick={() => setExpandedOrders(s => ({ ...s, [eKey+"||"+oKey]: !s[eKey+"||"+oKey] }))}
                      style={{ display: "flex", alignItems: "center", gap: 12, padding: "9px 18px 9px 52px", cursor: "pointer", background: someOrd ? "#fffbf7" : "#f9f9fb", borderLeft: isRestricted ? "3px solid #fca5a5" : "3px solid transparent", transition: "background 0.1s" }}
                      ...
                      <div style={{ fontSize: 11, color: "#9ca3af", flexShrink: 0, maxWidth: 180, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{orderGroup.accountEmail}</div>
                      <div style={{ width: 60, flexShrink: 0 }}>{qtyBox(oQty, oAvail)}</div>
                      <div style={{ width: 32, flexShrink: 0 }}>{soldBox(oSold)}</div>
                      <div style={{ fontSize: 13, fontWeight: 600, color: "#111827", width: 80, flexShrink: 0, textAlign: "right", fontVariantNumeric: "tabular-nums" }}>{fmt(oCost)}</div>
                        ...
                        {orderTickets.map((t, ti) => {
                          ...
                          return (
                            <div key={t.id}
                              ...
                              <div style={{ background: "white", border: "0.5px solid #e8e8ec", borderRadius: 6, padding: "3px 10px", fontSize: 12, fontWeight: 600, color: "#111827", whiteSpace: "nowrap", minWidth: 68, textAlign: "center" }}>
                                {seatLabel}
                              </div>
                              ...
                              <div style={{ display: "flex", gap: 4 }} onClick={e => e.stopPropagation()}>
                                {(t.qtyAvailable ?? t.qty) > 0 && !["Sold","Delivered","Completed"].includes(t.status) && (
