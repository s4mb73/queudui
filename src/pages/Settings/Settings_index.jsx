...
// ── Venue cleaner — strips time bleed, newlines, trailing commas ──────────────
function cleanVenue(raw) {
  return (raw || "")
    .replace(/^\s*\d{1,2}:\d{2}\s*(am|pm)\s*/i, "")  // strip leading time e.g. "11:00 am"
    .replace(/^(am|pm)\s*/i, "")                        // strip orphaned am/pm
    .replace(/\n/g, " ")                                // collapse newlines
...
export default function Settings({ settings, setSettings, tickets, setTickets, sales, setSales, events, findOrCreateEvent, notify, importParsed }) {
  ...
  async function importAll() {
    ...
    for (const p of bulkParsed) {
      ...
      const venueClean = cleanVenue(p.venue);  // ← strip time bleed / newlines / trailing comma
      ...
      if (findOrCreateEvent && eventName) {
        eventId = await findOrCreateEvent({
          name:     eventName,
          venue:    venueClean,     // ← use cleaned venue for dedup matching
          date:     p.date || "",
          time:     p.time || "",
      ...
      const base = {
        ...
        date:           p.date || "",
        time:           p.time || "",
        venue:          venueClean,   // ← store cleaned venue on the ticket
        orderRef:       p.orderRef || "",
        notes:          "",
